...
describe King, for: 'king' do
  describe '#controlled_square' do
    ...
    it 'shouldnt fail' do
      ...
      expect([corner_UL_tb, corner_UL_tb[0, 0]]).to contain_exact_positions([0, 1], [1, 0], [1, 1])
    end

    let(:corner_DR_tb) { Board.new '8/8/8/8/8/8/6K1/7K' }
    it 'shouldnt fail' do
      expect { corner_DR_tb[7, 7].controlled_square(corner_DR_tb) }.to_not raise_error
      expect([corner_DR_tb, corner_DR_tb[7, 7]]).to contain_exact_positions([7, 6], [6, 7], [6, 6])
    end
  end
end
