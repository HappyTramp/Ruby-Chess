...
describe VerifySpecialMoves, for: 'special_moves' do
  ...
  describe '#can_en_passant?' do
    ...
    let(:white_en_passant_edge_case) { Game.new Board.new '8/8/8/2PpP3/8/8/8/8' }
    ...
    it 'edge case with 2 pawn wich can en passant' do
      white_en_passant_edge_case.history.add_entry({piece: Pieces::init(:p, [3, 3]), from: [1, 3], to: [3, 3]})
      expect(white_en_passant_edge_case.can_en_passant?(:w)).to eq([
