...
describe Check, for: 'check' do
  ...
  describe '#in_check?' do
    let(:bishop_gw)   { Game.new '8/8/2b2q2/2R5/3PK3/8/8/8 w' }
    let(:knight_gw)   { Game.new '8/3b4/8/6n1/3PK3/8/8/8 w'   }
    let(:queen_gb)    { Game.new '8/3b4/8/3k4/8/3Q4/8/8 b'    }
    let(:pawn_gb)     { Game.new '8/8/4b3/3k4/4P3/4Q3/8/8 b'  }
    let(:nothing_gw)  { Game.new '8/8/4b3/4r3/8/3KQ3/8/8 w'   }
    let(:nothing_gb)  { Game.new '8/8/4b3/3k4/8/4Q3/8/8 b'    }

    context 'when king is attacked' do
      it { expect(bishop_gw.in_check?(:w)).to be true }
      it { expect(knight_gw.in_check?(:w)).to be true }
      it { expect(queen_gb.in_check?(:b)) .to be true }
      it { expect(pawn_gb.in_check?(:b))  .to be true }
    end

    context 'when king is not attacked' do
      it { expect(nothing_gw.in_check?(:w)).to be false }
      it { expect(nothing_gb.in_check?(:b)).to be false }
    end
  end
