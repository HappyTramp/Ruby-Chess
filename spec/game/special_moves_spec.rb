...
describe SpecialMoves, for: 'special_moves' do
  describe '#detect_promotion' do
    ...
    let(:promo_white_g) { Game.new '3P4/8/8/8/8/8/8/8 w' }
    let(:promo_black_g) { Game.new '8/8/8/8/8/8/8/3p4 b' }
    ...
    context 'when the king rook as moved before' do
      specify do
        ...
        expect(king_rook_moved_white_g.detect_castle)
          .to eq([Move.new(type_: :castle, side: :long)])
      end
      specify do
    ...
    context 'when the queen rook as moved before' do
      specify do
        ...
        expect(queen_rook_moved_white_g.detect_castle)
          .to eq([Move.new(type_: :castle, side: :short)])
      end
      specify do
    ...
    context 'when all conditions are fulfilled' do
      specify do
        expect(right_condition_white_g.detect_castle)
          .to eq([Move.new(type_: :castle, side: :short),
                  Move.new(type_: :castle, side: :long)])
      end
      specify do
  ...
  describe '#exec_castle' do
    ...
    before do
      white_short_castle_g.exec_castle(Move.new(type_: :castle, side: :short))
      black_short_castle_g.exec_castle(Move.new(type_: :castle, side: :short))
      white_long_castle_g.exec_castle(Move.new(type_: :castle, side: :long))
      black_long_castle_g.exec_castle(Move.new(type_: :castle, side: :long))
    end
